...
pub mod error;
pub use crate::layers::correctors::reed_solomon::parameters::error::Error;
...
impl Parameters {
    ...
    pub fn new(
        ...
        } else if total_num_shards == 0 {
            Err(Error::InvalidTotalShards { shard_count: total_num_shards, minimum_shards: 0 })
        ...
        } else if checksums.len() != total_num_shards as usize {
    ...
    pub fn from_data_buffer(data: &[u8]) -> Result<Self, Error> {
        ...
        let checksums: Vec<u32> = reader.read_u32_le_vec(total_num_shards)
            ...
            .try_into()
            .map_err(|_| Error::InvalidParameter("checksums"))?;
    ...
    /// |-------------|------------|-------------------|--------------------|--------------|
    /// | `[u32; N]`  | `u32`      | `u32`             | `u32`              | `u32`        |
    pub fn to_data_buffer(self, data: &mut Vec<u8>) {
        self.checksums
            .into_iter()
            .for_each(|checksum| data.extend_from_slice(&checksum.to_le_bytes()));

        data.extend_from_slice(&(self.data_len as u32).to_le_bytes());
        data.extend_from_slice(&(self.num_data_shards as u32).to_le_bytes());
        data.extend_from_slice(&(self.total_num_shards as u32).to_le_bytes());
        data.extend_from_slice(&(self.shard_size as u32).to_le_bytes());
    }
        ...
        all_prepared_shards
            ...
            .try_for_each(|(index, shard_opt)| {
                match shard_opt {
                    ...
                    None => Err(Error::MissingShard { missing_shard: index }),
                }
            })?;
