use crate::layers::correctors::reed_solomon::{DATA_LEN_MIN, DATA_LEN_MAX, Error, Parameters};
use crate::layers::{correctors::{Correctable, Level}, ValueBuf};
use reed_solomon_erasure::Field;

// -------------------------------------------------------------------------------------------------
//
/// Reed-Solomon encoder that uses [Darren Li](https://github.com/darrenldl), [Michael
...
impl<V: Correctable> ReedSolomon<V> {
    #[inline]
    #[must_use]
    pub fn is_protectable<'b>(data: &ValueBuf<'b>) -> bool {
    ...
    pub fn protect<'b>(data: ValueBuf<'b>) -> Result<ValueBuf<'b>, Error> {
        // Remember the size of the data block before padding, parity data, or the data block
        // length is added:
        let data_len = data.len();
        ...
        if data_len < DATA_LEN_MIN {
            ...
            let metadata = Parameters::new(
                ...
                num_data_shards,
                data_len,
                ReedSolomon::<V>::compute_checksums(&data, shard_size)
            )?;
            ...
            metadata.to_data_buffer(&mut data);
    ...
    pub fn recover<'b>(data: ValueBuf<'b>) -> Result<ValueBuf<'b>, Error> {
        // Remember the size of the data block before padding, parity data, or the data block
        // length is added:
        let data_len = data.len();
        ...
        if data_len < DATA_LEN_MIN {
            ...
            if corrupted_shards.is_empty() {
                ...
                // database:
                Ok(ValueBuf::from_recovered_data(recovered_data))
            }
        }
    }
    ...
    fn num_parity_shards(num_data_shards: usize) -> usize {
        ...
        match V::level() {
            Level::Basic    => 1,
            Level::Standard => (num_data_shards >> 2).max(1), // 25%
...
impl<V> ReedSolomon<V> {
    ...
    fn recommended_shard_size(data_len: usize) -> usize {
        if data_len == 0 {
            ...
            let shard_size = (1 << shard_log2).min(65_536);
    ...
    #[inline]
    fn num_data_shards(shard_size: usize, data_len: usize) -> usize {
    ...
    fn compute_checksums(
        ...
        data
            .chunks(shard_size)
            .map(|shard| crc32fast::hash(shard))
            .collect()
    }
}
...
mod shard_sizes {
    ...
    fn power_of_two_sizes() {
        ...
        for &size in &test_sizes {
            ...
            assert!(
                shard_size >= 16 && shard_size <= 65_536,
