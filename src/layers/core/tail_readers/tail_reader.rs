//! A byte-buffer reader that reads right-to-left from an immutable slice of bytes. This helps
...
impl<'b> TailReader<'b> {
    ...
    /// Creates a new `TailReader` from a `Bytes`.
    ///
    /// The reader is positioned at the end of the vector, ready to read backwards.
    #[inline]
    #[must_use]
    pub fn from_bytes(bytes: &'b Bytes<'b>) -> Self {
        Self::from(bytes)
    }

    /// Returns the current byte position of the `TrailReaderMut` when reading backwards
...
impl AsRef<[u8]> for TailReader<'_> {
...
impl<'b> From<&'b Bytes<'b>> for TailReader<'b> {
