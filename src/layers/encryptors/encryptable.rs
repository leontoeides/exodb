...
pub trait Encryptable {
    ...
    /// * `ReadOnly` · Use this when the data being written is always already encrypted. Data will
    ///   be decrypted on read.
    /// * `WriteOnly` · Encrypt on write and return encrypted data for furtherance, for example, to
    ///   another node.
    ...
    fn encryption_direction() -> &'static crate::layers::descriptors::Direction;
}