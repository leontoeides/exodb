...
pub struct KeySet(pub(crate) HashSet<Vec<u8>>);
...
impl KeySet {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self(HashSet::<Vec<u8>>::with_capacity(capacity))
    }
    ...
    pub fn intersection(self, other: &impl ReadableKeySet) -> Self {
        let intersection: HashSet<Vec<u8>> = self.0
            .into_iter()
    ...
    pub fn union(self, other: KeySet) -> Self {
        let union_result: HashSet<Vec<u8>> = self.0
            .into_iter()
            .chain(other.0)
            .collect();
    ...
    pub fn difference(self, other: &impl ReadableKeySet) -> Self {
        let difference: HashSet<Vec<u8>> = self.0
            .into_iter()
            .filter(|member| !other.contains(member))
            .collect();
...
impl std::ops::Deref for KeySet {
    ...
    /// Dereferences a `KeySet` into its underlying `HashSet<Vec<u8>>` collection.
