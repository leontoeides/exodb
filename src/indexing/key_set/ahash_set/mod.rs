//! An index `KeySet` help manages non-unique indexes. This implementation is powered by
//! [Tom Kaitchuck](https://github.com/tkaitchuck)'s [ahash](https://crates.io/crates/ahash) crate.
...
///
/// This implementation is powered by [Tom Kaitchuck](https://github.com/tkaitchuck)'s
/// [ahash](https://crates.io/crates/ahash) and [David Koloski](https://crates.io/users/djkoloski)'s
...
/// * Backed by efficient data structures like `HashSet` or deserialized on demand.
...
pub struct KeySet(pub(crate) HashSet<Vec<u8>, RandomState>);
...
impl KeySet {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self(HashSet::<Vec<u8>, RandomState>::with_capacity(capacity))
    }
    ...
    #[inline]
    #[must_use]
    pub fn into_inner(self) -> HashSet<Vec<u8>, RandomState> {
        self.0
    }
    ...
    pub fn intersection(self, other: &impl ReadableKeySet) -> Self {
        let intersection: HashSet<Vec<u8>, RandomState> = self.0
            .into_iter()
    ...
    pub fn union(
        ...
        let union_result: HashSet<Vec<u8>, RandomState> = self.0
            .into_iter()
            .chain(other.0)
            .collect();
    ...
    pub fn difference(self, other: &impl ReadableKeySet) -> Self {
        let difference: HashSet<Vec<u8>, RandomState> = self.0
            .into_iter()
            .filter(|member| !other.contains(member))
            .collect();
...
impl std::ops::Deref for KeySet {
    type Target = HashSet<Vec<u8>, RandomState>;

    /// Dereferences a `KeySet` into its underlying `HashSet<Vec<u8>, RandomState>` collection.
...
impl IntoIterator for KeySet {
    type Item = Vec<u8>;
    type IntoIter = std::collections::hash_set::IntoIter<Vec<u8>>;
...
impl<'i> IntoIterator for &'i KeySet {
    type Item = &'i Vec<u8>;
    type IntoIter = std::collections::hash_set::Iter<'i, Vec<u8>>;
