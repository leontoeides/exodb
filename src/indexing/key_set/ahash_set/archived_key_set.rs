//! Implementations for `ahash_set::ArchivedKeySet`, the zero-copy key-set view.

use crate::indexing::key_set::{ahash_set::{ArchivedKeySet, KeySet}, ReadableKeySet};
use rkyv::collections::swiss_table::{ArchivedHashSet, map::Keys};
use rkyv::{hash::FxHasher64, vec::ArchivedVec};
...
impl ArchivedKeySet {
    ...
    #[inline]
    #[must_use]
    pub fn iter(&self) -> Keys<'_, ArchivedVec<u8>, (), FxHasher64> {
        self.0.iter()
    }
    ...
    #[inline]
    #[must_use]
    pub const fn into_inner(self) -> ArchivedHashSet<ArchivedVec<u8>> {
        self.0
    }
...
impl std::ops::Deref for ArchivedKeySet {
    type Target = ArchivedHashSet<ArchivedVec<u8>>;

    /// Dereferences an `ArchivedKeySet` into its underlying `ArchivedHashSet<ArchivedVec<u8>>`
    /// collection.
...
impl<'i> IntoIterator for &'i ArchivedKeySet {
    ...
    type IntoIter = Keys<'i, ArchivedVec<u8>, (), FxHasher64>;
