...
pub fn assert_order_preserved<T: Codec<T> + Ord + std::fmt::Debug>(values: &[T]) {
    let mut encoded = values
        .iter()
        .map(|v| T::encode(v).expect("failed to encode"))
        .collect::<Vec<_>>();
    ...
    let decoded = encoded
        .iter()
        .map(|b| T::decode(b).expect("failed to decode"))
        .collect::<Vec<_>>();
    ...
    ($name:ident, $ty:ty, [$($val:expr),+ $(,)?]) => {
        ...
        fn $name() {
            ...
            let mut encoded = inputs
                .iter()
                .map(|x| <$ty as Codec<$ty>>::encode(x).expect("encoding failed"))
                .collect::<Vec<_>>();
            ...
            let decoded = encoded
                .into_iter()
                .map(|b| <$ty as Codec<$ty>>::decode(&b).expect("decoding failed"))
                .collect::<Vec<_>>();
